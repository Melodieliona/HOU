...
pub fn apply_normalize_an(constraint: Constraint, subst: &PersistentSubst) -> Vec<State> {
    ...
    let mapping: HashMap<_, _> = rhs_vars
        .iter()
        .enumerate()
        .map(|(i, y)| (y.clone(), lhs_vars[i].clone()))
        .collect();
    ...
    // Sei inner = t' x...
    let inner = lhs_vars[n..].iter().fold(t_prime, |acc, var| {
        Term::App(Box::new(acc), Box::new(Term::BVar(var.clone())))
    });

    // Neue rechte Seite
    let new_rhs = lhs_vars
        .iter()
        .rev()
        .fold(inner, |acc, var| Term::Abs(var.clone(), Box::new(acc)));
...
fn collect_lambdas(term: &Term) -> (Vec<String>, Term) {
    let mut vars = Vec::new();
    let mut t = term.clone();
    while let Term::Abs(p, body) = t {
        vars.push(p.clone());
        t = *body;
    }
    (vars, t)
...
fn rename_vars(term: Term, mapping: &HashMap<String, String>) -> Term {
    match term {
        ...
        Term::Abs(p, b) => {
            ...
            Term::Abs(new_p, Box::new(rename_vars(*b, mapping)))
        }
    }
}
